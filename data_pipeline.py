...
class LipReadingDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # This handles edge cases where path might not be resolved
        if not os.path.isabs(video_path):
            video_path = str(self.data_dir / video_path)
        
        try:
            # CRITICAL FIX: Use allow_pickle=False and mmap_mode=None for safety
            # This prevents hanging on corrupted files and avoids memory-mapped file issues
...
def create_dataloader(
    ...
    dataset = LipReadingDataset(data_dir, config=config, phase=phase, split=split)
    dataset.train(training)
    dataset.set_epoch(epoch)
    LOGGER.info(
        "Creating dataloader: data_dir=%s batch_size=%s epoch=%s training=%s phase=%s split=%s",
        data_dir, batch_size, epoch, training, phase.name, split or "all"
    )
    ...
    dataloader = DataLoader(
        ...
        timeout=30 if num_workers > 0 else 0,  # 30 second timeout for worker processes
    )
...
def text_to_phonemes(text: str) -> List[str]:
    ...
    for char in text.lower():
        if char in GRAPHEME_TO_PHONEME:
            phonemes.extend(GRAPHEME_TO_PHONEME[char])
    return phonemes
