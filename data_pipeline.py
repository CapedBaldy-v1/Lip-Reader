...
class LipReadingDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"[Dataset] Loaded {len(self.samples)} samples (phase: {phase.name})")
        LOGGER.info("Dataset loaded: %s samples (phase=%s).", len(self.samples), phase.name)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            log_exception(LOGGER, f"Failed to load video sample: {video_path}")
            video = np.zeros(
...
def create_dataloader(
    ...
    dataset = LipReadingDataset(data_dir, config=config, phase=phase)
    dataset.train(training)
    dataset.set_epoch(epoch)
    LOGGER.info(
        "Creating dataloader: data_dir=%s batch_size=%s epoch=%s training=%s phase=%s",
        data_dir, batch_size, epoch, training, phase.name
    )
    ...
    dataloader = DataLoader(
        ...
        sampler=sampler,
        shuffle=(not training and sampler is None),
