#!/usr/bin/env python3
"""
Train Swin-VALLR with WORD-ALIGNED data.
Uses precise frame-to-word alignment for better training.
"""
...
class WordAlignedDataset(Dataset):
    """Dataset with word-level frame alignment."""
    ...
    def __init__(self, data_dir: str, split: str = 'train'):
        ...
        print(f"  Loaded {len(self.samples)} {split} samples (word-aligned)")
    ...
    def __getitem__(self, idx):
        ...
        # Load frame-aligned label
        label_path = self.data_dir / sample['label_path']
        with open(label_path, 'r', encoding='utf-8') as f:
            text = f.read().strip()
        
        # Load alignment info (optional, for debugging)
        alignment_path = self.data_dir / sample['alignment_path']
        ...
        return video, text, sample['video_id'], alignment
...
def collate_fn(batch):
    ...
    videos, texts, video_ids, alignments = zip(*batch)
    ...
    return videos, targets, target_lengths, list(texts), list(video_ids), list(alignments)
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch):
    ...
    for videos, targets, target_lengths, texts, video_ids, alignments in pbar:
        # Move to device
...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for videos, targets, target_lengths, texts, video_ids, alignments in tqdm(dataloader, desc="Validating"):
            # Move to device
            videos = videos.to(device)
            targets = targets.to(device)
            target_lengths = target_lengths.to(device)
            ...
            for i in range(logits.size(0)):
                ...
                predictions.append({
                    ...
                    'target': target_text,
                    'cer': cer,
                    'num_words': alignments[i]['num_words'],
                    'num_frames': alignments[i]['num_frames']
                })
...
def main():
    print("="*70)
    print("TRAINING SWIN-VALLR WITH WORD-ALIGNED DATA")
    print("="*70)
    
    # Configuration
    data_dir = 'dataset_word_aligned'
    batch_size = 2
    num_epochs = 50
    learning_rate = 1e-4
    ...
    # Create datasets
    print("\n[1/5] Loading word-aligned datasets...")
    train_dataset = WordAlignedDataset(data_dir, 'train')
    val_dataset = WordAlignedDataset(data_dir, 'val')
    ...
    print(f"  ✓ Word-level alignment: ENABLED")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            for pred in predictions[:2]:
                print(f"  Video: {pred['video_id']} ({pred['num_words']} words, {pred['num_frames']} frames)")
    ...
    # Load best model
    checkpoint = torch.load('best_model_word_aligned.pt')
    model.load_state_dict(checkpoint['model_state_dict'])
    
    # Evaluate
    val_loss, val_cer, predictions = validate(model, val_loader, criterion, device)
    
    print(f"\nBest Model Performance (Word-Aligned):")
    ...
    for pred in predictions:
        print(f"\n  Video: {pred['video_id']}")
        print(f"  Words: {pred['num_words']}, Frames: {pred['num_frames']}")
    ...
    with open('training_results_word_aligned.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✓ Results saved to training_results_word_aligned.json")
    print(f"✓ Best model saved to best_model_word_aligned.pt")
    print()
